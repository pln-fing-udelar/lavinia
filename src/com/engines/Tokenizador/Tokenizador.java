package com.engines.Tokenizador;
 ...
 */
public class Tokenizador extends CasAnnotator_ImplBase{
	...
	public void typeSystemInit(TypeSystem aTypeSystem) throws AnalysisEngineProcessException {	    
	    ...
	    if (token == null){
	    	logger.log(Level.SEVERE, "No se encuentra el tipo Token");
	    	throw new AnalysisEngineProcessException(
	        AnnotatorInitializationException.TYPE_NOT_FOUND,
	        new Object[]{getClass().getName(), "com.engines.Token"});
	    }
	...
	public void process(CAS aCAS) throws AnalysisEngineProcessException {
			...
			int indfin = result.indexOf(endline);			
			int indice=0, temp;
			String line;
	        ...
	        while (indfin>=0) {	        		
	        	...
	        	line = result.substring(0, indfin);        	
	        	temp=text.indexOf(line,indice);	        	
	        	
	        	if (temp>=0){
	        		createAnnotation(aCAS,temp,temp+line.length());
	        		indice=temp+line.length();
	        	}		        	
	        	
	        	if ((indfin+1)<result.length()){
	        		result=result.substring(indfin+1,result.length());	        		
	        		indfin = result.indexOf(endline);
	        	}else
	        		indfin=-1;	        	
	        }	  			
	}		

	private void createAnnotation(CAS aCAS, int aBeginPos, int aEndPos) {	
		AnnotationFS annot = aCAS.createAnnotation(token, aBeginPos, aEndPos);
