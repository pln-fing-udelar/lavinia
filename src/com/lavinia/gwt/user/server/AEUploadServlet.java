...
public class AEUploadServlet extends HttpServlet{ 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException{ 
		...
		String AEname = request.getParameter("id");
		...
		while (iter.hasNext()) {			
	   		...
	   		if (!item.isFormField()) {	  	   			
	        	...
	        	if (fileName != null && !fileName.equals("")) {	        			        		
	        		File uploadedFile = new File(path+barra+AEname+".xml");	        		
	        		try {
						item.write(uploadedFile);
					} catch (Exception e) {
						String msg = "Error al escribir archivo "+path+barra+AEname+".xml";
						if (apconf.getLang().equals("es"))
							msg = "Error writing file "+path+barra+AEname+".xml";
						log.error(msg);						
						throw new ServletException();
					}
